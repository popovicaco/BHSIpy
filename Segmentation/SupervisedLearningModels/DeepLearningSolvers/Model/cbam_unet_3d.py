...
from tensorflow.keras.layers import Input, Conv3D, Reshape, concatenate, Conv2D, MaxPooling3D, Conv3DTranspose, BatchNormalization, Add, Activation, Dropout, Flatten, Multiply, Dense, Layer, GlobalAveragePooling3D, GlobalMaxPooling3D
...
class CBAM_UNet:
  """
  Class for Deep Learning Hyperspectral Segmentation with a 3D-CBAM-UNet.
  ...
  def decoding_layers_building_blocks(self, units, in_layer, concat_layer):
      ...
      concat_layer = self.cbam_block(concat_layer)
  ...
  def build_3d_unet(self):
    '''
    This function is a tensorflow realization of a 3D-CBAM-U-Net model. 
    '''
  ...
  def train(self):
    ...
    if self.utils.continue_training == True:
        ...
        unet = load_model('saved_models/cbam_unet_best_model.h5', custom_objects=custom_objects)
        unet.summary()
  ...
  def predict(self, new_data = None):
    ...
    '''
    unet = self.build_3d_unet()
    unet.load_weights('saved_models/cbam_unet_best_model.h5')
