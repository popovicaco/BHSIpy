...
from tensorflow.keras.layers import Input, Conv3D, Reshape, concatenate, Conv2D, MaxPooling3D, UpSampling3D, Conv3DTranspose, BatchNormalization, Add, Activation, Dropout, Lambda, Multiply, Flatten, Layer, GlobalAveragePooling3D, GlobalMaxPooling3D, Dense
...
class CBAM_Attention_UNet:
  """
  Class for Deep Learning Hyperspectral Segmentation with an CBAM Attention Gate UNet model.
  Reference: U-Net-based Models for Skin Lesion Segmentation: More Attention and Augmentation.
  ...
  def build_3d_attention_unet(self):
    '''
    This function is a tensorflow realization of a 3D-CBAM-AG-U-Net model. 
    '''
  ...
  def train(self):
    ...
    if self.utils.continue_training == True:
        ...
        attention_unet = load_model('saved_models/cbam_attention_unet_best_model.h5', custom_objects=custom_objects)
  ...
  def predict(self, new_data = None):
    ...
    '''
    attention_unet = self.build_3d_attention_unet()
    attention_unet.load_weights('saved_models/cbam_attention_unet_best_model.h5')
