...
from tensorflow.keras.layers import Input, Reshape, concatenate, Conv2D, MaxPool2D, Conv2DTranspose, BatchNormalization, Add, Activation, Dropout, Flatten
...
class UNet_2D:
  """
  Class for Deep Learning Hyperspectral Segmentation with a traditional 2D-UNet (the baseline model, similar to the one by Ronneberger et al, 2015)
  ...
  def encoding_layers_building_blocks(self, units, in_layer, pooling_layer=True):
      ...
      '''
      convolution_layer = Conv2D(units, (3, 3), padding='same', kernel_initializer='he_normal')(in_layer)
      convolution_layer = BatchNormalization()(convolution_layer)
      convolution_layer = Activation('relu')(convolution_layer)
      convolution_layer = Conv2D(units, (3, 3), padding='same', kernel_initializer='he_normal')(convolution_layer)
      convolution_layer = BatchNormalization()(convolution_layer)
      convolution_layer = Activation('relu')(convolution_layer)
      if pooling_layer == True:
          out_layer = MaxPool2D(pool_size=(2, 2), padding='same')(convolution_layer)
  ...
  def decoding_layers_building_blocks(self, units, in_layer, concat_layer):
      ...
      '''
      upsampling_layer = Conv2DTranspose(units, (3, 3), strides=(2, 2), padding='same', kernel_initializer='he_normal')(in_layer)
      upsampling_layer = concatenate([upsampling_layer, concat_layer], axis=3)
      convolution_layer = Conv2D(units, (3, 3), padding='same', kernel_initializer='he_normal')(upsampling_layer)
      convolution_layer = BatchNormalization()(convolution_layer)
      convolution_layer = Activation('relu')(convolution_layer)
      convolution_layer = Conv2D(units, (3, 3), padding='same', kernel_initializer='he_normal')(convolution_layer)
      convolution_layer = BatchNormalization()(convolution_layer)
  ...
  def build_2d_unet(self):
    '''
    This function is a tensorflow realization of a simple baseline 3D U-Net model (2015, 2016). 
    '''
    input_layer = Input((self.utils.resized_x_y, self.utils.resized_x_y, self.utils.num_components_to_keep))
    ...
    output_layer = Conv2D(self.utils.n_features, (1, 1))(up_sampling_output_layer_4)
    output_layer = Reshape((self.utils.resized_x_y, self.utils.resized_x_y, self.utils.n_features))(output_layer)
